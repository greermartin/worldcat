...
def get_libraries(oclc, api_key):    
    url = host + oclc + "?wskey=" + api_key + "&location=us&maximumLibraries=100&format=json" # form URL
...
with open("batch.csv", "r") as infile, \
    open("batch_count_us.csv", "w") as outfile:
...
# create a writer object to add rows to your outfile
    fieldnames.append('US_COUNT') # name for new column
